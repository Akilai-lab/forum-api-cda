...
import BatteredWomen from './components/Forum/batteredWomen.js';
import ChildrenAid from './components/Forum/childrenAid.js';
...
function AppRouter() {
  /* Ici nous avons le routing de l'application qui par défaut une fois la connexion réussi nous redirigera sur le component BatteredWomen */
  ...
  return (
    ...
    <Routes>
      ...
      <Route path="/login" element={auth ? <Navigate to={"/BatteredWomen"}/> : renderForForm()}/>
      <Route element={<ProtectedRoutes UserAuth={auth}/>}>
        <Route path='/BatteredWomen' element={<BatteredWomen />} />
        <Route path='/ChildrenAid' element={<ChildrenAid />} />
